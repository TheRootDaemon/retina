...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{name: "trace", level: LevelTrace, logFunc: (*Logger).Trace},
		{name: "debug", level: LevelDebug, logFunc: (*Logger).Debug},
		{name: "info", level: LevelInfo, logFunc: (*Logger).Info},
		{name: "warn", level: LevelWarn, logFunc: (*Logger).Warn},
		{name: "error", level: LevelError, logFunc: (*Logger).Error},
	}
...
func TestEnabled(t *testing.T) {
	tests := []struct {
		...
		{name: "info when trace", level: LevelTrace, arg: LevelInfo, want: true},
		{name: "warn when warn", level: LevelWarn, arg: LevelWarn, want: true},
		{name: "trace when warn", level: LevelWarn, arg: LevelTrace, want: false},
		{name: "error when warn", level: LevelWarn, arg: LevelError, want: true},
...
func TestZapLevel(t *testing.T) {
	tests := []struct {
		...
		{name: "unknown defaults to info", level: Level(999)},
		{name: "negative unknown defaults to info", level: Level(-100)},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.envXDG != "" {
				...
				t.Setenv("XDG_STATE_HOME", tt.envXDG)
			}
			if tt.envHome != "" {
...
func TestNewFromConfigWritesToFile(t *testing.T) {
	...
	l := NewFromConfig(cfg)
	l.Info("file write test %d", 42)
	_ = l.Close()

	content, err := os.ReadFile(filepath.Join(dir, "hop/hop.log"))
	require.NoError(t, err)
	assert.Contains(t, string(content), "file write test 42")
}
